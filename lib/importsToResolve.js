...
function importsToResolve(url) {
    ...
    let request = utils.urlToRequest(url);
    ...
    WEBPACK: CORIDYN:
    2018-11-16 Coridyn:
    We (wiwo) don't tend to put a file extension on our imports but
    bootstrap-sass does so this breaks our `wiwo-bootstrap` file.
    
    Disable this shortcut for now to see if we can at least get things building.
    */
    // if (ext === ".scss" || ext === ".sass") {
    //     return [request, url];
    // }
    
    // Trim off the extension so it has to try the partials
    request = request.replace(/\.s[ac]ss/, '');
    
    /* END OF DISABLED SHORT-CIRCUIT */

...
function sassLoader(content) {
     ...
     * DEBUG DEBUG DEBUG
     *
     * Just return empty content to take sass compilation out
     */
    if (process.env.NO_SASS){
        // console.log('################# NO_SASS #################');
        callback(null, "");
        return;
    // } else {
    //     // console.log('################# WITH_SASS #################');
    ...
    //     let start = Date.now();
    //     let orig = callback;
    //     console.log(`\n1 (sass-loader): ${chalk.yellow(resourcePath)}`);
    //
    //     callback = function(){
    //         let end = Date.now();
    //
    //         console.log(`\n2 (sass-loader): ${resourcePath} time=${end - start}`);
    //
    ...
    function addNormalizedDependency(file) {
        // // node-sass returns POSIX paths but we need platform-specific paths added as dependencies
        // self.dependency(path.normalize(file));
        
        /*
        2018-12-23
        Don't add our virtual "glob" files because we can't get timestamps
        for them and breaks webpack's caching behaviour
        */
        if (!file.includes('*')){
            // node-sass returns POSIX paths but we need platform-specific paths added as dependencies
            self.dependency(path.normalize(file));

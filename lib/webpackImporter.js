...
function webpackImporter(resourcePath, resolve, addNormalizedDependency) {
    ...
    function resolveGlobImport(url, prev){
        ...
        if (startsRelativeRE.test(globPath)){
            ...
            let relPath = path.join(basePrev, globPath);
            ...
            if (fs.existsSync(relPath)){
                ...
                let depId = globPath.split('/', 1)[0] || '';
                if (deps[depId]){
                    // If it does then resolve the glob against that module path.
                    let depPath = deps[depId];
                    globPath = path.join(depPath, globPath);
                } else {
        ...
        if (startsRelativeRE.test(globPath)){
            let globContext = path.join(basePrev, globPath);
        ...
        if (result && result.length){
            ...
            let content = [`/* dir="${basePrev}" pattern="${globPattern}" */`]
                .concat(
...
function getBowerDeps(){
    if (!_deps){
        ...
        _deps = WiwoDep.getBowerDependencyEntries('bower.json');
    }
    return _deps;
}
// End of Wiwo

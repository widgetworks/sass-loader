...
const NO_GLOB/*: boolean | 'warn' */ = (function(rawValue){
    let result = false;
    if (rawValue.toLowerCase() == 'warn'){
        result = 'warn';
    } else if (rawValue) {
        result = true;
    }
...
function webpackImporter(resourcePath, resolve, addNormalizedDependency) {
    ...
    function wiwoResolve(url, prev){
        ...
        if (glob.hasMagic(url)){
            if (NO_GLOB === 'warn'){
                console.warn(`(@wiwo/sass-loader) glob in ${chalk.red(url)} (from ${prev})`);
            } else if (NO_GLOB){
                throw new Error(`(@wiwo/sass-loader) glob in ${url} (from ${prev})`);
            }
